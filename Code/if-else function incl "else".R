...
get_scores <- function(data){
  ...
  #CUSADOS
  if ("cusados_1" %in% colnames(data) == TRUE) { scores$cusados_score <-rowSums(data[,23:35])
  cusados_score <- data %>% 
...
ggplot(data = scores) +
  geom_histogram(aes( color = "Respiration 1")) +
  
hist(scores$PCL,breaks=40)
hist(scores$DQ5)
hist(scores$covid, breaks = 20) 
